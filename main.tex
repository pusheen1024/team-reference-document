...
\pagestyle{fancy}
\fancyfoot{}
\fancyhead[L]{Saratov State University: Anofrikova, Druzhkin, Potapkina}
\fancyhead[R]{\thepage}
...
\subsection{0-1 BFS}
\inputminted[breaklines]{cpp}{Graph algorithms/0-1 bfs.cpp}
\subsection{Floyd}
...
Пусть нужно вычислить $a^b$ по модулю, причём показатель степени не влезает в long long. Тогда $b$ вычисляется по модулю $M - 1$, а $a^b$ "--- уже по модулю $M$.
\subsection{CRT}
...
Повернуть систему координат на угол $\alpha$:\\
$(x, y) \rightarrow (x \cdot cos(\alpha) + y \cdot sin(\alpha), y \cdot cos(\alpha) - x \cdot sin(\alpha))$
\subsection{FFT}
\inputminted[breaklines]{python}{Maths/fft.cpp}
...
\inputminted[breaklines]{cpp}{Other algorithms/xor-basis.cpp}
